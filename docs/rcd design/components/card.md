...
Cards are built with as little markup and styles as possible, but still manage to deliver a ton of control and customization. Built with flexbox, they offer easy alignment and mix well with other Bootstrap components. They have no `margin` by default, so use [spacing utilities]({{ site.url_root }}/docs/{{ site.docs_version }}/utilities/spacing/) as needed.
...
Use our handful of [available sizing utilities]({{ site.url_root }}/docs/{{ site.docs_version }}/utilities/sizing/) to quickly set a card's width.
...
You can quickly change the text alignment of any card—in its entirety or specific parts—with our [text align classes]({{ site.url_root }}/docs/{{ site.docs_version }}/utilities/text/#text-alignment).
...
Add some navigation to a card's header (or block) with Bootstrap's [nav components]({{ site.url_root }}/docs/{{ site.docs_version }}/components/navs/).
...
{% endcapture %}
{% include partials/callout.html content=callout type="info" %}
...
Use the Bootstrap grid system and its [`.row-cols` classes]({{ site.url_root }}/docs/{{ site.docs_version }}/layout/grid/#row-columns) to control how many grid columns (wrapped around your cards) you show per row. For example, here's `.row-cols-1` laying out the cards on one column, and `.row-cols-md-2` splitting four cards to equal width across multiple rows, from the medium breakpoint up.
