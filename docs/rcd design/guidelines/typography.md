...
unique-name: rcd-design-guidelines-typography
title: 字体

permalink: /rcd-design/guidelines/typography
...
字体大小由下方公式计算生成，默认最小字体的大小为 .625rem。行高使用相同公式计算，默认最小行高为 .75rem。
...
{% capture example %}
{% for i in (1..4) %}
<h1 class="display-0{{ i }}">Display-0{{ i }}. 标题</h1>
{% endfor %}
{% endcapture %}
{% include partials/example.html content=example %}
